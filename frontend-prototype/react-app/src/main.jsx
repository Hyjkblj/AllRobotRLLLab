...
function MuJoCoViewport({ assetId, frame, pose, model }) {
  ...
  return <div className={`viewport real-viewport ${dragging ? "grabbing" : ""}`} onPointerDown={handlePointerDown} onPointerMove={handlePointerMove} onPointerUp={endPointerDrag} onPointerCancel={endPointerDrag} onWheel={handleWheel}>
    {imageUrl ? <img className="mujoco-render" src={imageUrl} alt={`MuJoCo 渲染的 ${model?.name || "机器人"} 网格`} /> : <div className="render-loading"><RefreshCw size={18} className="spin" /><span>{renderError || "正在初始化 MuJoCo 渲染器"}</span></div>}
    <div className="viewport-hint"><span className="live-dot" /> MuJoCo Renderer · {model?.robotId || "robot"}</div>
    <div className="render-source-tag">真实 MJCF 网格 · {pose?.qpos?.length || 0} qpos</div>
...
function App() {
  ...
  const [actionRoot, setActionRoot] = useState("");
  ...
  const refresh = async () => {
    ...
    try {
      ...
      setActionRoot(nextActions.root || "");
  ...
  const addKeyframe = async () => {
    if (!selectedId) return;
    try {
      const result = await api(`/actions/${selectedId}/keyframes`, { method: "POST", body: JSON.stringify({ frame, label: `${model?.robotId || "robot"} · F${String(frame).padStart(3, "0")}` }) });
  ...
  const exportMotion = async () => {
    ...
    try {
      ...
      anchor.href = url;
      anchor.download = `${selectedAsset?.name || model?.robotId || "robot-motion"}-edited.json`;
      anchor.click();
  ...
  return <div className="app-shell">
    ...
    <main className="main-shell">
      <header className="topbar">
        <div className="crumb"><span className="crumb-muted">Motion Lab</span><span className="crumb-divider">/</span><strong>{model?.robotId ? `${model.robotId} Motion Editor` : "Robot Motion Editor"}</strong></div>
      ...
      <section className="workspace-head">
        <div><p className="eyebrow">REAL ROBOT DATA / MUJOCO</p><h1>动作微调工作台</h1><p className="subhead">导入真实机器人模型，定位关键帧，使用 MuJoCo 限位进行逐关节调整。</p></div>
        <div className="head-metrics"><div><span>模型</span><strong>{model?.name || "—"}</strong></div><div><span>qpos</span><strong>{model?.nq || "—"}</strong></div><div><span>资产</span><strong>{assets.length || "—"}</strong></div></div>
      </section>

      <div className="work-grid">
        <section className="asset-panel panel-surface">
          <div className="panel-heading"><div><span className="panel-kicker">MOTION LIBRARY</span><h2>动作源</h2></div><Badge tone="blue">{model?.robotId || "robot"}</Badge></div>
          ...
          <div className="asset-footer"><span>{visibleAssets.length} 个资产</span><span className="asset-path">根目录 · {shortName(actionRoot, 28)}</span></div>
        </section>
        ...
        <section className="editor-panel">
          ...
          <MuJoCoViewport key={selectedId || "empty"} assetId={selectedId} frame={frame} pose={pose} model={model} />
        ...
        <aside className="inspector panel-surface">
          <div className="panel-heading"><div><span className="panel-kicker">INSPECTOR</span><h2>姿态检查</h2></div><Badge tone="silver">{model?.jointCount ?? "—"} DoF</Badge></div>
          ...
          {tab === "source" && <div className="inspector-body"><div className="source-block"><div className="source-title"><Archive size={16} /><span>真实 MJCF / URDF</span></div><code>{shortName(model?.modelPath, 46)}</code><div className="source-meta"><Badge tone="green">MJCF loaded</Badge><span>{model?.jointCount ?? "—"} movable joints</span></div></div><label className="upload-drop"><Upload size={18} /><span>导入本地 URDF</span><small>解析关节名称与限位，不替换服务模型</small><input type="file" accept=".urdf,.xml" onChange={importUrdf} /></label>{(localUrdf || urdf) && <div className="urdf-result"><div><strong>{localUrdf?.name || urdf?.path?.split(/[\\/]/).pop() || "robot.urdf"}</strong><span>{localUrdf?.joints?.length || urdf?.jointCount || 0} 个可动关节</span></div><X size={14} onClick={() => setLocalUrdf(null)} /></div>}<div className="source-block"><div className="source-title"><FileUp size={16} /><span>注册动作路径</span></div><input className="path-input" value={importPath} onChange={(event) => setImportPath(event.target.value)} placeholder="输入动作目录路径" /><button className="button button-light wide" onClick={registerPath}><FileUp size={14} /> 加入动作目录</button><small className="muted-note">服务只接受 MOTIONLAB_ACTION_ROOT 内的 .pt / .npz / .csv / .pkl。</small></div></div>}
        </aside>
      </div>

      <footer className="statusbar"><div><span className="status-label">MODEL</span><strong>{shortName(model?.modelPath, 58) || "loading"}</strong></div><div><span className="status-label">URDF</span><strong>{localUrdf?.name || urdf?.path?.split(/[\\/]/).pop() || "—"}</strong></div><div className="status-spacer" /><div className="status-note"><Gauge size={14} /> {pose ? `root z ${(pose.root?.z || 0).toFixed(3)} m` : "等待动作帧"}</div></footer>
    </main>
