...
def test_native_isaac_provider_has_platform_entrypoint_default(tmp_path: Path, monkeypatch) -> None:
    ...
    command = NativeIsaacLabProvider._require_command(
        "NATIVE_ISAAC_TRAIN_COMMAND",
        default=["python", "-m", "apps.isaac_tasks.entrypoint", "train"],
    )
    assert command == ("python", "-m", "apps.isaac_tasks.entrypoint", "train")


def test_native_isaac_provider_supports_only_implemented_tasks_by_default(monkeypatch) -> None:
    monkeypatch.delenv("NATIVE_ISAAC_SUPPORTED_TASKS", raising=False)
    ...
    monkeypatch.setenv("NATIVE_ISAAC_SUPPORTED_TASKS", "unitree_h1_19dof:h1_mimic")
    assert provider.supports(robot_id="unitree_h1_19dof", task_id="h1_mimic")
