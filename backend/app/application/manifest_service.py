...
def load_runtime_versions(path: Path) -> RuntimeVersions | None:
    ...
    try:
        ...
        values = {
            ...
            "unitree_mujoco_git": external.get("unitree_mujoco", {}).get("git_sha") or "pending",
            "gmr_git": external.get("gmr", {}).get("git_sha") or "pending",
            "gvhmr_git": external.get("gvhmr", {}).get("git_sha") or "pending",
